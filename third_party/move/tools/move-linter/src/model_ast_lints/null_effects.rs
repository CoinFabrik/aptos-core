...
//! This module implements an expression linter that checks for statements that do nothing. In particular, it looks for three kinds of statemtents:
//! First, assignments where the destination is nowhere:
//!   *(&mut 0) = pure_expression;
//!   *(&mut 1) = pure_expression;
//!   *(&mut (x + 1)) = pure_expression;
//! Second, statements consisting of a single expression that doesn't change change any state:
//!   0;
//!   x;
//!   x + 1;
//!   pure_function();
//! Third, it looks for complex expressions that contain internal state changes that are not propagated outside of themselves.
...
impl ExpChecker for NullEffects {
    fn get_name(&self) -> String {
        "null_effects".to_string()
    }
...
impl NullEffects {
    fn visit_subexpression(
        ...
        match expr {
            ExpData::Lambda(id, _, _, _, _) => {
                ...
                self.report(env, &env.get_node_loc(*id), "This statement has no effect");
            },
            ExpData::Mutate(id, left, right) => {
                if !Self::left_side_is_bitbucket(left, function) {
                    return;
                }
                ...
                if analyzer.classify_expression_purity(right, function) == EffectClass::Pure {
                    let env = function.env();
                    self.report(env, &env.get_node_loc(*id), "This assignment has no effect");
                }
            },
            ...
            ExpData::Sequence(id, seq) => {
                ...
                let mut analyzer = EffectsAnalyzer::new();
                let p = analyzer.classify_expression_purity(expr, function);
                if p == EffectClass::Pure {
                    let env = function.env();
                    if depth > 0 || env.get_node_type(*id) == Type::Tuple(vec![]) {
                        self.report(env, &env.get_node_loc(*id), "This statement has no effect");
                    }
    ...
    // Returns true if the left side of an assignment is `*(&mut x)`, where `x`
    // is any literal or the result of some operation.
    fn left_side_is_bitbucket(left: &ExpData, function: &FunctionEnv) -> bool {
        if let ExpData::Call(_, op, args) = left {
            ...
            match e {
                ...
                ExpData::Call(_, op, _) => match op {
                    ...
                    | Operation::Gt
                    | Operation::Ge
                    | Operation::Le => return true,
                    _ => {},
                },
                _ => {},
            }
            if let ExpData::Value(_, _) = e {
                return true;
            }
            let mut analyzer = EffectsAnalyzer::new();
